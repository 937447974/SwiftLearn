...
class YJNSFileManagerVC: UIViewController {
    ...
    let fileMagager = NSFileManager.defaultManager()
    ...
    func testLocatingSystemDirectories() {
        // 获取tmp目录
        let tmpURL = fileMagager.URLsForDirectory(NSSearchPathDirectory.CachesDirectory, inDomains: .UserDomainMask).first
        print(tmpURL)
        // tmp下创建临时目录
        do {
            let createURL = try fileMagager.URLForDirectory(.ItemReplacementDirectory, inDomain: .UserDomainMask, appropriateForURL: tmpURL, create: true)
            print(createURL)
        } catch {
            // 在捕获中，隐式携带error错误。
            print("未知错误：\(error)")
    ...
    func testLocatingApplicationGroupContainerDirectories() {
        ...
        let groupURL = self.fileMagager.containerURLForSecurityApplicationGroupIdentifier("idebtufuer")
        print(groupURL)
    }
    
    func testDiscoveringDirectoryContents() {
        print(try? self.fileMagager.contentsOfDirectoryAtPath(YJUtilsDirectory.homePath))
        let enumerator = self.fileMagager.enumeratorAtPath(YJUtilsDirectory.homePath)
        for url in enumerator! {
            print(url)
        }
        print(self.fileMagager.subpathsAtPath(YJUtilsDirectory.homePath))
        print(try? self.fileMagager.subpathsOfDirectoryAtPath(YJUtilsDirectory.homePath))
    }
